...
import { X, AlertTriangle, LayoutTemplate, ChevronDown, ClipboardList } from 'lucide-react';
...
const DEPT_DESC = {
  ...
  banks_social_research:   'تسجيل بيانات البحوث الاجتماعية',
  banks_student_contracts: 'تسجيل عقود الطلبة الجامعيين',
...
export default function CreateTaskModal({ onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    if (user?.dept_id) {
      getDepartments()
        .then(depts => {
          const dc = depts.find(d => d.id === user.dept_id);
          if (dc?.fields?.length) {
            setDeptConfig(dc);
            const init = {};
  ...
  function isDeptFormValid() {
    return deptConfig.fields.every(f => {
    ...
    try {
      ...
      if (deptConfig) {
        payload = {
          ...
          type:       'incoming',
          priority:   'normal',
          extra_data: { _form_id: deptConfig.id, ...extra },
        };
      } else {
        payload = { ...form };
      }
  ...
  const isSubmitDisabled = busy || (deptConfig ? !isDeptFormValid() : !form.title.trim());
      ...
      <div className="modal-box modal-lg" onClick={e => e.stopPropagation()}>
        <div className="modal-head">
          <h3 className="modal-title">
            {deptConfig ? deptConfig.label : t.createTask}
          </h3>
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{ fontSize: '0.82rem', color: 'var(--accent-hover)', fontWeight: 600, marginBottom: '0.15rem' }}>
                      {DEPT_DESC[deptConfig.id] || deptConfig.label}
                    </div>
